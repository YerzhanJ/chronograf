...
export const applySelections = (
  ...
  for (const template of templates) {
    ...
    let selectedTemplateValue = template.values.find(v => v.selected)

    if (!selectedTemplateValue) {
      template.values[0].selected = true
      selectedTemplateValue = template.values[0]
    }

    let localSelectedValue = selections[template.tempVar]

    if (!localSelectedValue) {
      localSelectedValue = selectedTemplateValue.value
    }
